...
const USE_GENERIC_LLM =
  ...
  (!!process.env.LLM_API_KEY && !!process.env.LLM_API_BASE && !!process.env.LLM_MODEL);
...
if (USE_GENERIC_LLM) {
  ...
  const original = await import(path.join(__dirname, 'index.claude.js')).catch(() => null);
  if (original?.main) {
    (original as any).main();
  } else {
    // If original module isn't available, inline minimal bootstrap to avoid crash.
    console.error('[agent-runner] Missing Claude runner. Please rebuild container or disable generic LLM mode.');
    process.exit(1);
  }
}
