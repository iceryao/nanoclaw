...
export class FeishuChannel implements Channel {
  ...
  async sendMessage(jid: string, text: string): Promise<void> {
    ...
    logger.info({ jid, receiveId, text: text.slice(0, 100) }, 'Sending Feishu message');
  ...
  private async handleMessage(event: FeishuMessageEvent): Promise<void> {
    ...
    if (!message || message.message_type !== 'text') {
      logger.debug({ messageType: message?.message_type }, 'Skipping non-text message');
      return;
    }
    ...
    try {
      ...
      logger.warn({ eventId: event.event_id }, 'Failed to parse Feishu message content');
      return;
    }
    ...
    if (message.mentions && message.mentions.length > 0) {
      for (const mention of message.mentions) {
        if (mention.key && mention.name) {
          ...
          const displayName = mention.name === 'NanoClaw' ? ASSISTANT_NAME : mention.name;
          content = content.replace(mention.key, `@${displayName}`);
        }
      }
      logger.debug({ content, mentions: message.mentions }, 'Processed Feishu mentions');
    }
